# TODO: Translation updated at 2021-06-19 20:09

translate crowdin strings:

    # game/core/02init.rpy:18
    old "{color=#E59400}{i}Убеждение не удалось!{/i}{/color}\n"
    new "{color=#00FF00}{i}Persuasione riuscita! {/i} {/color}\n"

    # game/core/02init.rpy:19
    old "{color=#00FF00}{i}Убеждение удалось!{/i}{/color}\n"
    new "{color=#00FF00}{i}Persuasione riuscita! {/i} {/color}\n"

    # game/core/02init.rpy:21
    old "{color=#00FF00}{i}Вы остались незамеченным!{/i}{/color}\n"
    new "{color=#00FF00}{i}sei passato inosservato! {/i} {/color}\n"

    # game/core/02init.rpy:22
    old "{color=#E59400}{i}Вас заметили!{/i}{/color}\n"
    new "{color=#E59400}{i}hai notato! {/i} {/color}\n"

    # game/core/02init.rpy:23
    old "{color=#E59400}{i}Слишком рискованно!{/i}{/color}\n"
    new "{color=#E59400}{i}Troppo rischioso! {/i} {/color}\n"

    # game/core/02init.rpy:25
    old "{color=#00FF00}{i}Алисе понравился массаж!{/i}{/color}\n"
    new "{color=#00FF00}{i}Alice ha adorato il massaggio! {/i} {/color}\n"

    # game/core/02init.rpy:26
    old "{color=#E59400}{i}Алисе не понравился массаж!{/i}{/color}\n"
    new "{color=#E59400}{i}ad Alice non piaceva il massaggio! {/i} {/color}\n"

    # game/core/02init.rpy:28
    old "{color=#00FF00}{i}Лизе понравился массаж!{/i}{/color}\n"
    new "{color=#00FF00}{i}Alice ha adorato il massaggio! {/i} {/color}\n"

    # game/core/02init.rpy:29
    old "{color=#E59400}{i}Лизе не понравился массаж!{/i}{/color}\n"
    new "{color=#E59400}{i}a Lisa non piaceva il massaggio! {/i} {/color}\n"

    # game/core/02init.rpy:31
    old "{color=#00FF00}{i}Маме понравился массаж!{/i}{/color}\n"
    new "{color=#00FF00}{i}mamma ha adorato il massaggio! {/i} {/color}\n"

    # game/core/02init.rpy:32
    old "{color=#E59400}{i}Маме не понравился массаж!{/i}{/color}\n"
    new "{color=#E59400}{i}alla mamma non piaceva il massaggio! {/i} {/color}\n"

    # game/core/02init.rpy:33
    old "{color=#00FF00}{i}Получилось!{/i}{/color}\n"
    new "{color=#00FF00}{i}Ha funzionato! {/i} {/color}\n"

    # game/core/02init.rpy:34
    old "{color=#E59400}{i}Не получилось!{/i}{/color}\n"
    new "{color=#E59400}{i}Non ha funzionato! {/i} {/color}\n"

    # game/core/02init.rpy:36
    old "{color=#00FF00}{i}Удалось сдержаться{/i}{/color}\n"
    new "{color=#00FF00}{i}riuscito a trattenere{/i}{/color}\n"

    # game/core/02init.rpy:37
    old "{color=#E59400}{i}Сдержаться не удалось{/i}{/color}\n"
    new "{color=#E59400}{i}Impossibile contenere{/i}{/color}\n"

    # game/core/02init.rpy:39
    old "{color=#00FF00}{i}Лизе понравился поцелуй!{/i}{/color}\n"
    new "{color=#00FF00}{i}Lisa adorava il bacio! {/i} {/color}\n"

    # game/core/02init.rpy:40
    old "{color=#E59400}{i}Лизе не понравился поцелуй!{/i}{/color}\n"
    new "{color=#E59400}{i}a Lisa non piaceva il bacio! {/i} {/color}\n"

    # game/core/02init.rpy:42
    old "{color=#00FF00}{i}Ей нравится!{/i}{/color}\n"
    new "{color=#00FF00}{i}Le piace! {/i} {/color}\n"

    # game/core/02init.rpy:43
    old "{color=#E59400}{i}Ей не нравится!{/i}{/color}\n"
    new "{color=#E59400}{i}Non le piace! {/i} {/color}\n"

    # game/core/02init.rpy:79
    old "ВС"
    new "DOM"

    # game/core/02init.rpy:79
    old "ВОСКРЕСЕНЬЕ"
    new "DOMENICA"

    # game/core/02init.rpy:79
    old "ПН"
    new "LUN"

    # game/core/02init.rpy:79
    old "ПОНЕДЕЛЬНИК"
    new "LUNEDÌ"

    # game/core/02init.rpy:79
    old "ВТ"
    new "MAR"

    # game/core/02init.rpy:79
    old "ВТОРНИК"
    new "MARTEDÌ"

    # game/core/02init.rpy:79
    old "СР"
    new "MER"

    # game/core/02init.rpy:79
    old "СРЕДА"
    new "MERCOLEDÌ"

    # game/core/02init.rpy:79
    old "ЧТ"
    new "GIO"

    # game/core/02init.rpy:79
    old "ЧЕТВЕРГ"
    new "GIOVEDÌ"

    # game/core/02init.rpy:79
    old "ПТ"
    new "VEN"

    # game/core/02init.rpy:79
    old "ПЯТНИЦА"
    new "VENERDÌ"

    # game/core/02init.rpy:79
    old "СБ"
    new "SAB"

    # game/core/02init.rpy:79
    old "СУББОТА"
    new "SABATO"

    # game/core/02init.rpy:120
    old "Управление"
    new "Gestione"

    # game/core/02init.rpy:120
    old "В данной игре предусмотрено управление с помощью клавиатуры. Вы можете сохраняться в любой момент. Быстрые клавиши по-умолчанию:\nF5 - сохранить, F8 - загрузить.\n\n\nКроме того, во время диалогов работают клавиши 1... 9 для различных вариантов ответов. Если вариант всего один, можно нажимать клавишу [[Space].\n\n\nДля переключения между комнатами можно воспользоваться клавишами 1... 7.\n\n\nОтключить интерфейс можно клавишами [[ ` ], [[ h ] или нажав среднюю клавишу мыши.\nЭто очень удобно, если область диалогов скрывает интересную часть изображения..."
    new "Questo gioco fornisce il controllo della tastiera. Puoi risparmiare in qualsiasi momento. Scorciatoie da tastiera predefinite:\nF5 - salva, F8 - carica.\n\n\nInoltre, durante i dialoghi, il 1... 9 per diverse opzioni di risposta. Se c'è solo un'opzione, puoi premere il tasto [[Space].\n\n\nPer passare da una sala all'altra, puoi usare i tasti 1 ... 7.\n\n\nÈ possibile disabilitare l'interfaccia premendo [[ ' ], [[ h ] o premendo il pulsante centrale del mouse.\nQuesto è molto comodo se l'area di dialogo nasconde una parte interessante dell'immagine ..."

    # game/core/02init.rpy:120
    old "Возможности"
    new "Possibilità di successo"

    # game/core/02init.rpy:120
    old "В процессе игры, во время диалогов и других действий, вы можете открыть для себя новые \"возможности\". Их механика немного похожа на \"задания\" или \"квесты\" из других игр, но есть некоторые отличия.\n\n\n\"Возможности\" могут открывать доступ к скрытым событиям или покупкам в интернет-магазине, к новым опциям в ноутбуке или действиям в некоторых комнатах. Читайте внимательно описание каждой \"возможности\" и вы ничего не упустите!"
    new "Durante il gioco, durante i dialoghi e altre azioni, puoi scoprire nuove \"opportunità\". Le loro meccaniche sono un po 'simili alle \"missioni\" o \"missioni\" di altri giochi, ma ci sono alcune differenze.\n\n\n\"Funzionalità\" può aprire l'accesso a eventi o acquisti nascosti nel negozio online, a nuove opzioni nel laptop o azioni in alcune stanze. Leggi attentamente la descrizione di ogni \"opportunità\" e non ti perderai nulla!"

    # game/core/02init.rpy:120
    old "Настроение"
    new "Umore"

    # game/core/02init.rpy:120
    old "Различные действия или фразы, сказанные Максом, могут вызывать соответствующую реакцию в виде смены настроения персонажа.\n\n\nЕсли у персонажа плохое настроение, он может отказаться с вами обсуждать некоторые темы.\n\n\nПостепенно, каждый час, настроение плавно стремится к нейтральному состоянию. Однако, если настроение очень плохое, оно будет долго восстанавливаться.\n\n\nЧтобы поднять настроение, можно подарить то, что нужно именно этому персонажу или же просто извиниться. А иногда и правильное слово во время разговора может значительно улучшить настроение."
    new "Varie azioni o frasi dette da Max possono causare una reazione corrispondente sotto forma di un cambiamento nell'umore del personaggio.\n\n\nSe un personaggio è di cattivo umore, può rifiutarsi di discutere alcuni argomenti con te.\n\n\nGradualmente, ogni ora, l'umore tende dolcemente a uno stato neutro. Tuttavia, se l'umore è molto cattivo, ci vorrà molto tempo per recuperare.\n\n\nPer rallegrarti, puoi dare ciò di cui hai bisogno a questo particolare personaggio o semplicemente scusarti. E a volte la parola giusta durante una conversazione può migliorare significativamente l'umore."

    # game/core/02init.rpy:120
    old "Шоу"
    new "Mostrare"

    # game/core/02init.rpy:120
    old "Скрытые камеры могут быть основным источником дохода. Чем больше аудитория, тем больше людей, готовых платить за просмотр.\n\n\nУвеличить аудиторию можно с помощью рекламы. Если на камерах не происходит ничего интересного - аудитория падает. Если в кадр попадают пикантные моменты, аудитория растёт.\n\n\nЕсли у вас есть сайт, вы можете зарабатывать на рекламных баннерах. Поэтому, чем больше аудитория, тем больше у вас денег.\n\n\n{i}{b}Внимание:{/b} В следующих версиях игры появятся VIP-пользователи с особыми просьбами. Если вы будете их выполнять, получите солидную прибавку к доходу.{/i}"
    new "Le telecamere nascoste possono essere la principale fonte di reddito. Più grande è il pubblico, più persone sono disposte a pagare per la visione.\n\n\nPuoi aumentare il tuo pubblico con gli annunci. Se non succede nulla di interessante sulle telecamere, il pubblico cade. Se i momenti piccanti entrano nell'inquadratura, il pubblico cresce.\n\n\nSe hai un sito web, puoi guadagnare sui banner pubblicitari. Quindi più grande è il pubblico, più soldi hai.\n\n\n{i}{b}Nota:{/b} Nelle versioni future del gioco, gli utenti VIP appariranno con richieste speciali. Se li esegui, riceverai un solido aumento del reddito. {/i}"

    # game/core/02init.rpy:128
    old "Значит, у тебя есть блог?"
    new "Quindi hai un blog?"

    # game/core/02init.rpy:128
    old "Слушай, насчёт блога..."
    new "Ascolta, sul blog..."

    # game/core/02init.rpy:128
    old "Насчёт твоего блога... А если не особо раздеваться?"
    new "Per quanto riguarda il tuo blog... E se non ti spogli davvero?"

    # game/core/02init.rpy:128
    old "Насчёт посуды..."
    new "Per quanto riguarda i piatti..."

    # game/core/02init.rpy:128
    old "Не возражаешь против компании?"
    new "Non ti dispiace l'azienda?"

    # game/core/02init.rpy:128
    old "Что читаешь?"
    new "Cosa leggi?"

    # game/core/02init.rpy:128
    old "Хотел извиниться за утренний инцидент..."
    new "Volevo scusarmi per l'incidente del mattino..."

    # game/core/02init.rpy:128
    old "Загораешь?"
    new "Abbronzatura?"

    # game/core/02init.rpy:128
    old "У меня для тебя обещанная вкусняшка!"
    new "Ho il delizioso promesso per te!"

    # game/core/02init.rpy:128
    old "Насчёт ванны ночью..."
    new "Per quanto riguarda il bagno di notte..."

    # game/core/02init.rpy:128
    old "А ты умеешь целоваться?"
    new "Sai come baciare?"

    # game/core/02init.rpy:128
    old "Я слышал, Эрик тебе новое бельё собирается купить?"
    new "Ho sentito che Eric ti comprerà biancheria intima nuova?"

    # game/core/02init.rpy:128
    old "Покажешь боди, которое тебе Эрик купит?"
    new "Puoi mostrarmi il body che Eric ti comprerà?"

    # game/core/02init.rpy:128
    old "Хотел узнать, хорошо ли тебе сидится?"
    new "Volevo sapere se stavi bene?"

    # game/core/02init.rpy:128
    old "Не слабо тебя отшлёпали!"
    new "Non debolmente sei stato attraversato!"

    # game/core/02init.rpy:128
    old "Ты не передумала о наказаниях?"
    new "Non hai ripensato alle punizioni?"

    # game/core/02init.rpy:128
    old "Отшлёпать тебя сейчас или..."
    new "Sculacciati ora o..."

    # game/core/02init.rpy:128
    old "Пора отшлёпать одну милую попку!"
    new "È tempo di sculacciare un carino!"

    # game/core/02init.rpy:128
    old "Мам, дай денег, пожалуйста..."
    new "Mamma, dammi dei soldi, per favore..."

    # game/core/02init.rpy:128
    old "Я продукты заказал!"
    new "Ho ordinato i prodotti!"

    # game/core/02init.rpy:128
    old "Мам, бассейн чист!"
    new "Mamma, la piscina è pulita!"

    # game/core/02init.rpy:128
    old "Что смотришь?"
    new "Cosa stai guardando?"

    # game/core/02init.rpy:128
    old "Насчёт случая с Лизой..."
    new "Per quanto riguarda il caso Lisa..."

    # game/core/02init.rpy:128
    old "Мам, а как учатся целоваться?"
    new "Mamma, come impari a baciare?"

    # game/core/02init.rpy:128
    old "Мам, Кира отправила меня к тебе..."
    new "Mamma, Kira mi ha mandato da te..."

    # game/core/02init.rpy:128
    old "С тобой можно?"
    new "Posso stare con te?"

    # game/core/02init.rpy:128
    old "Я присоединюсь?"
    new "Mi iscriverò?"

    # game/core/02init.rpy:128
    old "Мне нужны деньги..."
    new "Ho bisogno di soldi..."

    # game/core/02init.rpy:128
    old "Эрик, мы же договорились!"
    new "Eric, siamo d'accordo!"

    # game/core/02init.rpy:128
    old "Хочу рассказать тебе кое-что о Кире..."
    new "Voglio dirti qualcosa su Kira..."

    # game/core/02init.rpy:128
    old "Я хотел поговорить о Кире..."
    new "Volevo parlare di Kira..."

    # game/core/02init.rpy:128
    old "Да тут всегда хорошая погода..."
    new "Sì, c'è sempre bel tempo..."

    # game/core/02init.rpy:128
    old "Ага, как всегда..."
    new "Sì, come sempre..."

    # game/core/02init.rpy:128
    old "Да, шикарная!"
    new "Sì, stupendo!"

    # game/core/02init.rpy:128
    old "Кира, мне нужно научиться целоваться..."
    new "Kira, ho bisogno di imparare a baciare..."

    # game/core/02init.rpy:128
    old "Ну как, ты с мамой-то поговорила?"
    new "Bene, hai parlato con tua madre?"

    # game/core/02init.rpy:128
    old "Как отдыхается, тётя Кира?"
    new "Come riposa zia Kira?"

    # game/core/02init.rpy:128
    old "Насчёт фотосессии..."
    new "Per quanto riguarda il servizio fotografico..."

    # game/core/02init.rpy:128
    old "Понравились фотографии?"
    new "Ti piacciono le foto?"

    # game/core/02init.rpy:128
    old "Не злишься на меня, тётя Кира?"
    new "Non sei arrabbiata con me, zia Kira?"

    # game/core/02init.rpy:128
    old "Так когда будем снова фотографироваться, тётя Кира?"
    new "Quindi, quando saremo fotografati di nuovo, zia Kira?"

    # game/core/02init.rpy:128
    old "Тётя Кира, когда ты уже с мамой поговоришь?!"
    new "Zia Kira, quando parli con tua madre?!"

    # game/core/02init.rpy:128
    old "Я хотел спросить про тот случай из детства мамы..."
    new "Volevo chiedere di quell'incidente dell'infanzia di mia madre..."

    # game/core/02init.rpy:128
    old "Расскажи уже про тот случай из детства мамы..."
    new "Parlaci di quell'incidente dell'infanzia di mia madre..."

    # game/core/02init.rpy:128
    old "О школе..."
    new "A proposito della scuola..."

    # game/core/02init.rpy:128
    old "А ты чего так загораешь?"
    new "E perché prendi il sole in quel modo?"

    # game/core/02init.rpy:128
    old "Насчёт твоего парня..."
    new "Per quanto riguarda il tuo ragazzo..."

    # game/core/02init.rpy:128
    old "Насчёт этого случая утром..."
    new "Per quanto riguarda questo incidente al mattino..."

    # game/core/02init.rpy:128
    old "Хотел поговорить о Большом Максе..."
    new "Volevo parlare di Big Max..."

    # game/core/02init.rpy:128
    old "А ты у нас шалунья, оказывается..."
    new "E tu sei un, si scopre..."

    # game/core/02init.rpy:128
    old "Насчёт успеваемости..."
    new "Per quanto riguarda il rendimento scolastico..."

    # game/core/02init.rpy:128
    old "Ну как, ты подумала о моих условиях?"
    new "Bene, hai pensato ai miei termini?"

    # game/core/02init.rpy:128
    old "Помочь с уроками?"
    new "Aiuto con le lezioni?"

    # game/core/02init.rpy:128
    old "У тебя странный вид..."
    new "Hai uno strano sguardo..."

    # game/core/02init.rpy:128
    old "Может всё-таки поделишься своими переживаниями по поводу Алисы?"
    new "Puoi condividere i tuoi sentimenti su Alice?"

    # game/core/02init.rpy:128
    old "Массаж рук заказывала?"
    new "Hai ordinato un massaggio alle mani?"

    # game/core/02init.rpy:128
    old "Ну что, Лиза, готова?"
    new "Lisa è pronta?"

    # game/core/02init.rpy:128
    old "Ну что, готова?"
    new "E' pronto?"

    # game/core/02init.rpy:128
    old "Лиза, ты же любишь читать?"
    new "Lisa, ti piace leggere?"

    # game/core/02init.rpy:128
    old "Лиза, у меня для тебя особая книжка..."
    new "Lisa, ho un libro speciale per te..."

    # game/core/02init.rpy:128
    old "Рассказывай, что делали?"
    new "Raccontaci cosa hai fatto?"

    # game/core/02init.rpy:128
    old "Ну так и чему же тебя учили?"
    new "Quindi cosa ti è stato insegnato?"

    # game/core/02init.rpy:128
    old "Что новенького было на уроке?"
    new "Cosa c'era di nuovo in classe?"

    # game/core/02init.rpy:128
    old "Что нового мама с Эриком тебе рассказали?"
    new "Cosa ti hanno detto la nuova mamma ed Eric?"

    # game/core/02init.rpy:128
    old "Что нового узнала на уроке у мамы и Эрика?"
    new "Cosa hai imparato in classe da mia madre ed Eric?"

    # game/core/02init.rpy:128
    old "{i}урок поцелуев{/i}"
    new "{i}Lezione di bacio{/i}"

    # game/core/02init.rpy:128
    old "Насчёт Алекса..."
    new "Per quanto riguarda Alex..."

    # game/core/02init.rpy:128
    old "Есть успехи с Оливией?"
    new "Qualche successo con Olivia?"

    # game/core/02init.rpy:128
    old "Что-нибудь узнала про Оливию?"
    new "Hai imparato qualcosa su Olivia?"

    # game/core/02init.rpy:128
    old "Ну так, что там с трусиками Оливии?"
    new "Quindi cosa succede con le mutandine di Olivia?"

    # game/core/02init.rpy:128
    old "Ты позвала Оливию к нам?"
    new "Ci hai chiamato Olivia?"

    # game/core/02init.rpy:128
    old "Ну как, получилось рассказать всё Оливии?"
    new "Bene, come sei arrivato a raccontare tutto a Olivia?"

    # game/core/02init.rpy:128
    old "Нравится, что я спасаю твою попку от наказания?"
    new "Così ti salvo il dalla punizione?"

    # game/core/02init.rpy:128
    old "Учтите, я испытываю... некоторый подъём!"
    new "Intendiamomi, sto vivendo... un po 'di elevazione!"

    # game/core/02init.rpy:128
    old "Пошепчемся немного о моей сестрёнке?"
    new "Sussurriamo un po' di mia sorella?"

    # game/core/02init.rpy:128
    old "Что новенького, Оливия?"
    new "Cosa c'è di nuovo, Olivia?"

    # game/core/02init.rpy:128
    old "Рад тебя видеть, Оливия!"
    new "Bello vederti, Olivia!"

    # game/core/02init.rpy:219
    old "Одежда"
    new "Abbigliamento"

    # game/core/02init.rpy:219
    old "Книги"
    new "Libri"

    # game/core/02init.rpy:219
    old "Продукты"
    new "Generi alimentari"

    # game/core/02init.rpy:219
    old "Электроника"
    new "Elettronica"

    # game/core/02init.rpy:219
    old "Товары 18+"
    new "Prodotti 18+"

    # game/core/02init.rpy:219
    old "Косметика"
    new "Cosmesi"

    # game/core/02init.rpy:219
    old "Украшения"
    new "Orpelli"

    # game/core/02init.rpy:219
    old "Другое"
    new "Altro"

    # game/core/02init.rpy:230
    old "А у меня есть то, о чём ты мечтала..."
    new "E ho quello che hai sognato..."

    # game/core/02init.rpy:230
    old "У меня для тебя подарок {i}(Халат){/i}"
    new "Ho un regalo per te {i}(Accappatoio){/i}"

    # game/core/02init.rpy:230
    old "{color=#808080}У меня для тебя вкусняшка! \n (нужно выждать несколько дней){/color}"
    new "{color=#808080}Ho una buona cosa per te! \n (è necessario attendere qualche giorno){/color}"

    # game/core/02init.rpy:230
    old "У меня для тебя вкусняшка!"
    new "Ho un pasto delizioso per te!"

    # game/core/02init.rpy:230
    old "У меня есть кое-что запрещённое..."
    new "Ho qualcosa di proibito..."

    # game/core/02init.rpy:230
    old "Угадай: маленькое, чёрненькое..."
    new "Indovinate un po': piccolo, nero..."

    # game/core/02init.rpy:230
    old "У меня для тебя одна книжка..."
    new "Ho un libro per te..."

    # game/core/02init.rpy:230
    old "У меня снова для тебя книжка..."
    new "Ho di nuovo un libro per te..."

    # game/core/02init.rpy:230
    old "И снова у меня для тебя книжка..."
    new "Ancora una volta, ho un libro per te..."

    # game/core/02init.rpy:230
    old "У меня для тебя подарок {i}(Пижама){/i}"
    new "Ho un regalo per te {i}(Pigiama){/i}"

    # game/core/02init.rpy:230
    old "У меня есть кое-что, о чём мы беседовали..."
    new "Ho qualcosa di cui abbiamo parlato..."

    # game/core/02init.rpy:230
    old "{color=#808080}Прикупил для тебя немного сладенького! \n (нужно выждать несколько дней){/color}"
    new "{color=#808080}Comprato del dolce per te! \n (è necessario attendere qualche giorno){/color}"

    # game/core/02init.rpy:230
    old "Прикупил для тебя немного сладенького!"
    new "Comprato un po 'dolce per te!"

    # game/core/02init.rpy:230
    old "Мам, я купил тебе одежду полегче!"
    new "Mamma, ti ho comprato vestiti più leggeri!"

